...
sealed class XiaoRiUpdateStatus {
    object Idle : XiaoRiUpdateStatus()
    object Checking : XiaoRiUpdateStatus()
    ...
    data class NoUpdate(val version: String) : XiaoRiUpdateStatus()
    data class Error(val message: String) : XiaoRiUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<XiaoRiUpdateStatus>(XiaoRiUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = XiaoRiUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        XiaoRiUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = XiaoRiUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is XiaoRiUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as XiaoRiUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is XiaoRiUpdateStatus.Idle, is XiaoRiUpdateStatus.Checking, is XiaoRiUpdateStatus.NoUpdate, is XiaoRiUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is XiaoRiUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is XiaoRiUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/XiaoRiApp/XiaoRi/releases/download/${currentStatus.version}/xiaori.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is XiaoRiUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is XiaoRiUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is XiaoRiUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/XiaoRiApp/XiaoRi/releases/latest")
            ...
            if (shouldShow) {
                ...
                try {
                    val changelogUrl =
                        URL("https://github.com/XiaoRiApp/XiaoRi/releases/download/$tagWithPrefix/changelog.json")
